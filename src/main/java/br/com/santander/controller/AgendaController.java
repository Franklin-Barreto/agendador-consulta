...
public class AgendaController {
	...
	@GetMapping("medico/{id}/horarios")
	public ResponseEntity<?> consultaHorariosDisponiveis(@PathVariable Integer id){
		List<Agenda> horariosDisponiveis = agendaService.buscaHorarioDisponiveis(id);
		return ResponseEntity.ok(horariosDisponiveis);
	}
	
	@PutMapping("/{id}/paciente/{pacienteId}")
	public ResponseEntity<?> agendarConsulta(@PathVariable Integer id, @PathVariable Integer pacienteId){
		Agenda consulta = agendaService.marcaConsulta(id,pacienteId);
		return ResponseEntity.ok(consulta);
	}
	
	
	@GetMapping("medico/{id}/consultas")
	public List<Agenda> buscaConsultasMedico(@PathVariable Integer id){
		return agendaService.buscaConsultasMedico(id);
	}
